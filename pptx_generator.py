...
def add_fulltime_slide(prs, comparison_df, team_name, opponent_name, team_color, opponent_color,
    ...
    stat_specs = [
        ...
        (opponent_wins, f"categories won\nby {opponent_name}", opponent_color),
...
def generate_report(
    ...
    comparison_csv: path (or file-like) to the comparison CSV from
        stats_processor.build_team_comparison(), with columns
        ['Metric', team_name, opponent_name] and rows covering at
    ...
    team_player_stats_csv / opponent_player_stats_csv: optional paths
        ...
        per-metric player-breakdown slides - those slides are skipped
        if not provided.
